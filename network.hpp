...
using namespace std;
//executive level
#define executive_d_num 10 //the number of deterministic neurons
#define executive_z_num 3 //the number of latent neurons
#define executive_tau 8.0 //time constant of deterministic neurons
#define executive_W 1.0 //meta-prior
#define executive_W1 1.0 //meta-prior at initial time step
//associative level
#define associative_d_num 20 //the number of deterministic neurons
#define associative_z_num 8 //the number of latent neurons
#define associative_tau 4.0 //time constant of deterministic neurons
#define associative_W 1.0 //meta-prior
#define associative_W1 1.0 //meta-prior at initial time step
//neuromodulation
#define neuromodulation_d_num 10 //the number of deterministic neurons
#define neuromodulation_z_num 1 //the number of latent neurons
#define neuromodulation_tau 4.0 //time constant of deterministic neurons
#define neuromodulation_W 1.0 //meta-prior
#define neuromodulation_W1 1.0 //meta-prior at initial time step
//sensory level
#define exteroceptive_d_num 20 //the number of deterministic neurons
#define exteroceptive_z_num 2 //the number of latent neurons
#define exteroceptive_tau 2.0 //time constant of deterministic neurons
#define exteroceptive_W 1.0 //meta-prior
#define exteroceptive_W1 1.0 //meta-prior at initial time step

#define proprioceptive_d_num 10 //the number of deterministic neurons
#define proprioceptive_z_num 2 //the number of latent neurons
#define proprioceptive_tau 2.0 //time constant of deterministic neurons
#define proprioceptive_W 1.0 //meta-prior
#define proprioceptive_W1 1.0 //meta-prior at initial time step

#define interoceptive_d_num 10 //the number of deterministic neurons
#define interoceptive_z_num 1 //the number of latent neurons
#define interoceptive_tau 2.0 //time constant of deterministic neurons
#define interoceptive_W 1.0 //meta-prior
#define interoceptive_W1 1.0 //meta-prior at initial time step
...
#define x_proprio_num 2 //the dimension of proprioception
#define x_intero_num 1
#define x_nm_tau 1.0

#define ETA 0.000001 //used to avoid null computation
...
string trained_model_foldername = "learning_model"; //set foldername of trained model which you want to read
...
void set_target_data(vector<int>& length, vector<vector<vector<double> > >& target, string data_name){
    ...
    for ( int s = 0; s < sequence_size; ++s ) {
        int time_length = int(target[s].size());
        ostringstream ostr;
        ...
        while (getline(ifs_target[s], line)) {
            ...
            for (int i=0; i<index_size ;++i){
                if(t < time_length) target[s][t][i] = stod(strvec.at(i));
            }
            ++t;
        }
        length[s] = t;
...
public:
    ...
    Output(int seq_num, int initial_length, int out_num, int hd_num, string& fo_path_param, string& fo_path_gen, string fi_path_param){
        ...
        if(fi_path_param=="learning"){
            ...
            stringstream str;
            str << "./" << trained_model_foldername << "/" << fi_path_param << "/weight_ohd_" << trained_model_index << ".txt";
    ...
    void backward(vector<vector<vector<double> > >& hd, vector<vector<vector<double> > >& target, int length, int t, int s){
        ...
        double normalize = 1.0/(length*out_size*seq_size);
        for(int i=0;i<out_size;++i){
    ...
    void backward_nm(vector<vector<vector<double> > >& hd, vector<vector<vector<double> > >& target, vector<vector<vector<double> > >& sigma, int length, int t, int s){
        //compute gradient of internal state
        double normalize = 1.0/(length*out_size*seq_size);
        for(int i=0;i<out_size;++i){
            //negative log-likelihood
            prediction_error[s][t][i] = 0.5*(pow((target[s][t][i]-output[s][t][i])/sigma[s][t][i],2)+log(2*PI)+2.0*log(sigma[s][t][i]))*normalize;
            grad_internal_state_output[s][t][i] = -(1-pow(output[s][t][i],2))*(target[s][t][i]-output[s][t][i])/pow(sigma[s][t][i],2)*normalize;
            for(int j=0;j<hd_size;++j){
                grad_weight_ohd_each_sequence[s][i][j] += grad_internal_state_output[s][t][i]*hd[s][t][j]; //summation over time steps
            }
        }
    ...
    void update_parameter_radam_weight(double alpha, double adam1, double adam2, double rho, double r_store, double m_adam1, double l_adam2, double weight_decay){
    ...
    void update_parameter_adam_weight(double alpha, double adam1, double adam2, double m_adam1, double v_adam2, double weight_decay){
    ...
    void save_sequence_prediction(vector<vector<vector<double> > >& target, int length, int s, int epoch){
    ...
    void save_sequence_error(int length, int s, int epoch){
...
public:
    ...
    vector<vector<vector<double> > > internal_state_output;
    vector<vector<vector<double> > > output;
    vector<vector<vector<double> > > previous_internal_state_output;
    //vector<vector<vector<double> > > previous_output;
    vector<vector<vector<double> > > grad_internal_state_output;
    vector<vector<vector<double> > > prediction_error; //negative log-likelihood
    //initialize variables
    OutputNM(int seq_num, int initial_length, int out_num, int hd_num, double time_constant, string& fo_path_param, string& fo_path_gen, string fi_path_param){
        seq_size = seq_num;
        init_length = initial_length;
        out_size = out_num;
        hd_size = hd_num;
        ...
        adam_m_weight_ohd.assign(out_num, vector<double>(hd_num, 0));
        adam_v_weight_ohd.assign(out_num, vector<double>(hd_num, 0));
        tau.assign(out_num, time_constant); //used in backprop at higher level
        internal_state_output.assign(seq_num, vector<vector<double> >(initial_length, vector<double>(out_num, 0)));
        output.assign(seq_num, vector<vector<double> >(initial_length, vector<double>(out_num, 0)));
        previous_internal_state_output.assign(seq_num, vector<vector<double> >(initial_length, vector<double>(out_num, 0)));
        //previous_output.assign(seq_num, vector<vector<double> >(initial_length, vector<double>(out_num, 0)));
        grad_internal_state_output.assign(seq_num, vector<vector<double> >(initial_length, vector<double>(out_num, 0)));
        prediction_error.assign(seq_num, vector<vector<double> >(initial_length, vector<double>(out_num, 0)));
        //initialization
        if(fi_path_param=="learning"){
    ...
    void forward(vector<vector<vector<double> > >& hd, int t, int s){ 
        ...
        double store1=0; 
        for(int i=0;i<out_size;++i){
            if(t != 0) previous_internal_state_output[s][t][i] = internal_state_output[s][t-1][i];
            //previous_output[s][t][i] = exp(previous_internal_state_output[s][t][i]);
            store1=0;
            for(int j=0;j<hd_size;++j){
                store1 += weight_ohd[i][j] * hd[s][t][j];
            }
            internal_state_output[s][t][i] = (1.0-1.0/tau[i])*previous_internal_state_output[s][t][i]+1.0/tau[i]*(store1);
            output[s][t][i] = exp(internal_state_output[s][t][i])+S_ETA;
        }
    }
    void forward_change_sensory_gain(vector<vector<vector<double> > >& hd, vector<vector<vector<double> > >& sensory_gain, int t, int s){ 
        //hd: input from higher neurons, t: time step, s: sequence
        double store1=0; 
        for(int i=0;i<out_size;++i){
            if(t != 0) previous_internal_state_output[s][t][i] = internal_state_output[s][t-1][i];
            ...
            internal_state_output[s][t][i] = (1.0-1.0/tau[i])*previous_internal_state_output[s][t][i]+1.0/tau[i]*(store1);
            output[s][t][i] = sensory_gain[s][t][i]*exp(internal_state_output[s][t][i])+S_ETA;
        }
    }
    void backward(vector<vector<vector<double> > >& hd, vector<vector<vector<double> > >& target, vector<vector<vector<double> > >& mean, int length, int t, int s){
        //compute gradient of internal state
        double normalize = 1.0/(length*out_size*seq_size);
        for(int i=0;i<out_size;++i){
            //negative log-likelihood
            if(t==length-1){
                grad_internal_state_output[s][t][i] = (1.0-pow((target[s][t][i]-mean[s][t][i])/output[s][t][i],2))*normalize;
            }else{
                grad_internal_state_output[s][t][i] = (1.0-pow((target[s][t][i]-mean[s][t][i])/output[s][t][i],2))*normalize+(1.0-1.0/tau[i])*grad_internal_state_output[s][t+1][i];
            }
            for(int j=0;j<hd_size;++j){
                grad_weight_ohd_each_sequence[s][i][j] += 1.0/tau[i]*grad_internal_state_output[s][t][i]*hd[s][t][j]; //summation over time steps
            }
        }
    }
    void sum_gradient(int s){
    ...
    void save_sequence_prediction(int length, int s, int epoch){
        save_generated_sequence(fo_path_generation, "output", output, length, s, epoch);
    }
};
...
public:
    ...
    vector<vector<vector<double> > > prediction_error; //negative log-likelihood
    //initialize variables
    StochasticOutput(int seq_num, int initial_length, int out_num, int hd_num, string& fo_path_param, string& fo_path_gen, string fi_path_param){
        seq_size = seq_num;
        init_length = initial_length;
        mean_size = out_num;
        out_size = 2*out_num;
    ...
    void forward(vector<vector<vector<double> > >& hd, int t, int s){ 
        ...
        for(int i=0;i<mean_size;++i){
            ...
            output[s][t][i] = tanh(store1);
            output[s][t][i+mean_size] = exp(store2)+S_ETA;
        }
    }
    void backward(vector<vector<vector<double> > >& hd, vector<vector<vector<double> > >& target, int length, int t, int s){
        //compute gradient of internal state
        double normalize = 1.0/(length*mean_size*seq_size);
        for(int i=0;i<mean_size;++i){
            //negative log-likelihood
...
public:
    ...
    //initialize variables
    PVRNNLayer(int seq_num, int initial_length, int d_num, int hd_num, int z_num, double w, double w1, double time_constant, string& fo_path_param, string& fo_path_gen, string fi_path_param){
        seq_size = seq_num;
        init_length = initial_length;
        d_size = d_num;
        hd_size = hd_num;
        ...
        previous_internal_state_d.assign(seq_num, vector<vector<double> >(initial_length, vector<double>(d_num, 0))); //if not 0, initial priors can be updated through optimization of synaptic weights 
        ...
        if(fi_path_param=="learning"){
            ...
            str_pmd << "./" << trained_model_foldername << "/" << fi_path_param << "/weight_pmd_" << trained_model_index << ".txt";
            str_psd << "./" << trained_model_foldername << "/" << fi_path_param << "/weight_psd_" << trained_model_index << ".txt";
            str_dz << "./" << trained_model_foldername << "/" << fi_path_param << "/weight_dz_" << trained_model_index << ".txt";
            str_dhd << "./" << trained_model_foldername << "/" << fi_path_param << "/weight_dhd_" << trained_model_index << ".txt";
            str_dd << "./" << trained_model_foldername << "/" << fi_path_param << "/weight_dd_" << trained_model_index << ".txt";
            str_bias << "./" << trained_model_foldername << "/" << fi_path_param << "/bias_" << trained_model_index << ".txt";
            ...
            /*if you want to set initial adaptive vector externally (by default, initial adaptive vectors are initialized with initial priors)
            ...
            for(int s=0;s<seq_num;++s){
                ...
                str_a_mu << "./" << trained_model_foldername << "/" << fi_path_param << "/a_mu_" << setw(7) << setfill('0') << s << "_" << trained_model_index << ".txt";
                str_a_sigma << "./" << trained_model_foldername << "/" << fi_path_param << "/a_sigma_" << setw(7) << setfill('0') << s << "_" << trained_model_index << ".txt";
    ...
    void backward(vector<vector<vector<double> > >& hd, vector<vector<double> >& weight_ld, vector<vector<vector<double> > >& grad_internal_state_lower, vector<double>& tau_lower, int length, int t, int s){ //weight_ld: connections to lower neurons, grad_internal_state_lower: gradient of internal states of lower neurons
        ...
        double normalize = 1.0/(length*z_size*seq_size);
        for(int i=0;i<d_size;++i){
        ...
        double meta_prior;
        (t==0) ? (meta_prior=W1) : (meta_prior=W);
        for(int i=0;i<z_size;++i){
            ...
            wkld[s][t][i] = meta_prior*kld[s][t][i];
            store1=0;
            for(int k=0;k<d_size;++k){
                store1 += 1.0/tau[k]*weight_dz[k][i]*grad_internal_state_d[s][t][k]; //gradient of z
            }
            grad_a_mu[s][t][i] = (1.0-pow(q_mu[s][t][i],2))*(store1+meta_prior*(q_mu[s][t][i]-p_mu[s][t][i])/pow(p_sigma[s][t][i],2)*normalize);
            grad_a_sigma[s][t][i] = q_sigma[s][t][i]*eps[s][t][i]*store1+ meta_prior*(-1.0+pow(q_sigma[s][t][i],2)/pow(p_sigma[s][t][i],2))*normalize;
            grad_internal_state_p_mu[s][t][i] = (1.0-pow(p_mu[s][t][i],2))*meta_prior*(p_mu[s][t][i]-q_mu[s][t][i])/pow(p_sigma[s][t][i],2)*normalize;
            grad_internal_state_p_sigma[s][t][i] = meta_prior*(1.0-(pow(p_mu[s][t][i]-q_mu[s][t][i],2)+pow(q_sigma[s][t][i],2))/pow(p_sigma[s][t][i],2))*normalize;
            for(int k=0;k<d_size;++k){
    ...
    void update_parameter_radam_posterior(int t, int s, double alpha, double adam1, double adam2, double rho, double r_store, double m_adam1, double l_adam2){
        double m_store=0, l_store=0;
        /*
        double rho_inf = 2.0/(1.0-adam2)-1.0;
        double rho = rho_inf-2.0*(epoch+1)*pow(adam2,epoch+1)/(1.0-pow(adam2,epoch+1));
    ...
    void update_parameter_adam_posterior(int t, int s, double alpha, double adam1, double adam2, double m_adam1, double v_adam2){
        double m_store=0, v_store=0, delta_store=0;
        for(int i=0;i<z_size;++i){
            adam_m_a_mu[s][t][i] = adam1*adam_m_a_mu[s][t][i]+(1-adam1)*grad_a_mu[s][t][i];
            adam_v_a_mu[s][t][i] = adam2*adam_v_a_mu[s][t][i]+(1-adam2)*pow(grad_a_mu[s][t][i],2);
    ...
    void save_sequence_prediction(int length, int s, int epoch){
        ...
        save_generated_sequence(fo_path_generation, "d", d, length, s, epoch);
    }
    void save_sequence_error(int length, int s, int epoch){
...
public:
    ...
    vector<vector<vector<double> > > wkld;
    //initialize variables
    PVRNNTopLayer(int seq_num, int initial_length, int d_num, int z_num, double w, double w1, double time_constant, string& fo_path_param, string& fo_path_gen, string fi_path_param){
        seq_size = seq_num;
        init_length = initial_length;
        d_size = d_num;
        z_size = z_num;
    ...
    void update_parameter_radam_weight_pd_dz(double alpha, double adam1, double adam2, double rho, double r_store, double m_adam1, double l_adam2, double weight_decay){
        double m_store=0, l_store=0;
        /*
        double rho_inf = 2.0/(1.0-ADAM2)-1.0;
        double rho = rho_inf-2.0*(epoch+1)*pow(adam2,epoch+1)/(1.0-pow(adam2,epoch+1));
...
public:
    ...
    void update_parameter_adam_posterior(int s, double alpha, double adam1, double adam2, double m_adam1, double v_adam2){
        double m_store=0, v_store=0, delta_store=0;
        /*
        double m_adam1 = 1.0/(1.0-pow(adam1,epoch+1));
        double v_adam2 = 1.0/(1.0-pow(adam2,epoch+1));
...
public:
    ...
    void er_backward(vector<vector<vector<double> > >& target, int t, int s, int window_size){
        //compute gradient of internal state
        double normalize = 1.0/(out_size*window_size);
        for(int i=0;i<out_size;++i){
            //prediction_error[s][t][i] = 0.5*pow(target[s][t][i]-output[s][t][i],2)*normalize;
            grad_internal_state_output[s][t][i] = -(1-pow(output[s][t][i],2))*(target[s][t][i]-output[s][t][i])*normalize;
        }
    }
    void er_backward_nm(vector<vector<vector<double> > >& target, vector<vector<vector<double> > >& sigma, int t, int s, int window_size){
        //compute gradient of internal state
        double normalize = 1.0/(out_size*window_size);
        for(int i=0;i<out_size;++i){
            //negative log-likelihood
...
public:
    ...
    //vector<vector<vector<double> > > prediction_error;
    //initialize variables
    ER_OutputNM(int seq_num, int initial_length, int out_num, int hd_num, double time_constant, string fi_path_param){
        seq_size = seq_num;
        init_length = initial_length;
        out_size = out_num;
        hd_size = hd_num;
    ...
    void er_increment_timestep(int s){
        internal_state_output[s].emplace_back(vector<double>(out_size,0));
        output[s].emplace_back(vector<double>(out_size,0));
        previous_internal_state_output[s].emplace_back(vector<double>(out_size,0));
        //previous_output[s].emplace_back(vector<double>(out_size,0));
        grad_internal_state_output[s].emplace_back(vector<double>(out_size,0));
        //prediction_error[s].emplace_back(vector<double>(out_size,0));
    }
    void er_forward(vector<vector<vector<double> > >& hd, int t, int s){ 
    ...
    void er_forward_change_sensory_gain(vector<vector<vector<double> > >& hd, vector<vector<vector<double> > >& sensory_gain, int t, int s){ 
        //hd: input from higher neurons, t: time step, s: sequence
        double store1=0; 
        for(int i=0;i<out_size;++i){
            if(t != 0) previous_internal_state_output[s][t][i] = internal_state_output[s][t-1][i];
    ...
    void er_backward(vector<vector<vector<double> > >& target, vector<vector<vector<double> > >& mean, int last_window_step, int t, int s, int window_size){
        //compute gradient of internal state
...
public:
    ...
    //vector<vector<vector<double> > > wkld;
    //initialize variables
    ER_PVRNNLayer(int seq_num, int initial_length, int d_num, int hd_num, int z_num, double w, double w1, double time_constant, string fi_path_param){
        seq_size = seq_num;
        init_length = initial_length;
        d_size = d_num;
        hd_size = hd_num;
    ...
    void er_backward(vector<vector<double> >& weight_ld, vector<vector<vector<double> > >& grad_internal_state_lower, vector<double>& tau_lower, int last_window_step, int t, int s, int window_size){ //weight_ld: connections to lower neurons, grad_internal_state_lower: gradient of internal states of lower neurons
        int l_size = int(weight_ld.size());
        double store1=0, store2=0, store3=0, store4=0;
        double normalize = 1.0/(z_size*window_size);
        for(int i=0;i<d_size;++i){
            //compute gradient of internal state of deterministic neuron
            if(last_window_step==1){
                //error from lower neurons
    ...
    void er_backward_change_metaprior(vector<vector<double> >& weight_ld, vector<vector<vector<double> > >& grad_internal_state_lower, vector<double>& tau_lower, int last_window_step, int t, int s, int window_size, double metaprior){ //weight_ld: connections to lower neurons, grad_internal_state_lower: gradient of internal states of lower neurons
        int l_size = int(weight_ld.size());
        double store1=0, store2=0, store3=0, store4=0;
        double normalize = 1.0/(z_size*window_size);
        for(int i=0;i<d_size;++i){
    ...
    void er_save_sequence(string& path_save_generation, int length, int s, int epoch){
        ...
        save_generated_sequence(path_save_generation, "d", d, length, s, epoch);
...
public:
    ...
    //vector<vector<vector<double> > > wkld;
    //initialize variables
    ER_PVRNNTopLayer(int seq_num, int initial_length, int d_num, int z_num, double w, double w1, double time_constant, string fi_path_param){
        seq_size = seq_num;
        init_length = initial_length;
        d_size = d_num;
        z_size = z_num;
...
public:
    ...
    void er_backward(vector<vector<double> >& weight_lz, vector<vector<vector<double> > >& grad_internal_state_lower, vector<double>& tau_lower, int last_window_step, int t, int s, int window_size){ //weight_ld: connections to lower neurons, grad_internal_state_lower: gradient of internal states of lower neurons
        int l_size = int(weight_lz.size());
        double store1=0;
        double normalize = 1.0/(z_size*window_size);  //if using Output class (not Stochastic_Output), 1.0/z_size;
        for(int i=0;i<z_size;++i){
